    ...
    event.waitUntil(
        caches.open("app-cache").then(cache => {
            return Promise.all([
                ...
                fetch("/static/icons/circle-icon-192x192.png").then(res => cache.put("/static/icons/icon-192x192.png", res)),
                fetch("/static/icons/circle-icon-512x512.png").then(res => cache.put("/static/icons/icon-512x512.png", res))
            ]);
